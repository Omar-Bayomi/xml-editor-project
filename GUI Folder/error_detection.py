...
def error_check(path):
    ...
    output_path = filename + ' error_check.txt'
    with open(path, 'r+') as file, open(output_path, 'w') as output:
        ...
        for line in file:
            if Extra_flag == True: 
                output.write('error: extra content are added after the end of root tag\n')
                error = True
                break
            line_count += 1
                            ...
                            if opening == True:
                                    ...
                                    if len(tags_redundant_stack) != 0:
                                        ...
                                        if tags_stack[-1].name == tag_obj.name :
                                            ...
                                            # print error clause
                                            output.write('error using wrong closing \'</{}>\' at line {}, should be \'</{}>\'\n'.format(
        ...
        if len(tags_stack) != 0:
            ...
            while len(tags_redundant_stack) != 0:
                output.write('error: opening without closing \'<{}>\' at line {}\n'.format(tags_redundant_stack[-1].name,
                                                                                     tags_redundant_stack[-1].line_num))
                error = True                                                                     
                tags_redundant_stack.pop()

        elif error == False: # no error
            output.write("checked successfully: no errors\n")        
